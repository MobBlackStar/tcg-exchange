            ...
            @forelse($favorites as $card)
                <div class="tcard shadow-brick-magenta wishlisted">
                    <div class="titlebar" style="display: flex; justify-content: space-between; padding: 8px 15px; border-bottom: 4px solid var(--ink-c);">
                        ...
                        <!-- Un-heart button -->
                        <button class="btn-wishlist active" onclick="toggleWishlist(event, {{ $card->id }})" style="background: none; border: none; color: var(--a1); cursor: pointer;">
                    ...
                    <div class="body" style="display: flex; flex-direction: column; align-items: center; text-align: center; padding: 24px;">
                        <img src="{{ $card->image_url }}" style="width: 180px; border: 3px solid #000; margin-bottom: 20px;">
                        <h3 style="font-size: 1.1rem;">{{ $card->name }}</h3>
                        <a href="{{ route('cards.show', $card->id) }}" class="btn outline sm full" style="margin-top: 15px;"><span class="inner">View Market Data</span></a>
                    </div>
                </div>
            @empty
